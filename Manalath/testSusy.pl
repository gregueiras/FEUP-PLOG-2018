...
final_board([
    ...
    cell( 2, 5,whitePiece),cell( 4, 5, emptyCell),cell( 6, 5,whitePiece),cell( 8, 5, emptyCell),cell(10, 5, blackPiece),
...
findFirstNeighbors(Board,X,Y,_P,Color,OldRes, Res) :-
 findall((FX,FY),
  (
...
findAllNeighbors(Board,T,Color,Res) :-
  length(T,N),
  N >= 6,!,
  Res = T.

findAllNeighbors(Board,[(SX,SY)|T],Color,Res) :-
  %member((SX,SY), FirstNeighbors),
  findFirstNeighbors(Board,SX,SY,_S,Color,[(SX,SY)|T],Tmp),
  append(T,Tmp,R),
  findAllNeighbors(Board,R,Color,Res).
